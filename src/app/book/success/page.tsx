...
export default async function BookingSuccessPage({
  ...
  // 3. Get the error from the result
  const { orderId, projectBrief, error } = orderResult

  // 4. Check for and render the error message FIRST
  if (error) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center p-12 text-center">
        <h1 className="text-3xl font-bold tracking-tight text-red-600 sm:text-4xl">
          An Error Occurred
        </h1>
        <p className="mt-4 text-lg text-gray-600 dark:text-gray-400">
          Your payment was successful, but there was a problem creating your order in our system.
        </p>
        <p className="mt-2 text-sm text-gray-500">
          Please contact support immediately and quote this error: <br/>
          <strong className="text-gray-900 dark:text-gray-300">{error}</strong>
        </p>
        <Link
          href="/"
